...
**3. 白条面板里直接删这笔还款**（2026-09-07 用户提出，先记着）。现在在面板里点开一笔已经记过的还款，只能改金额和勾选，想撤回得跑到流水页去找那笔转账再删。要加的是一个红色的「删掉这笔还款」按钮，放在「取消」旁边，点了要确认一次。删除本身的行为已经是对的（`settles` 挂在还款那一侧，还款没了结清关系跟着消失、订单自动回到账单里），缺的只是这个入口。
