...
**1. 离线记账**（优先级最高）。现在断网只能**看**，记不了——写入直连数据库，没网就失败。要做的是一个补录队列：先写本地，联网后自动补上去。难点不在存，在**和现有的同步时序打架**：`store.ts` 里那套在途补丁 / 代际号 / 30 秒中止是按「写入立刻到云端」设计的，补录会让一笔记录在本地存在、云端不存在，refresh 回来时不能把它冲掉。动这块必须补 `store.test.ts` 的用例。
