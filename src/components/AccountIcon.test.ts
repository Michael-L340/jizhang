...
describe('账户图标', () => {
  ...
  it('白条圆标是「底板正中放一张透明底位图」，位图本身的居中在 png.test.ts 量像素守', () => {
    // 先后用过 emoji 💳（没居中）和自绘 SVG（居中但风格不配）。现在是用户生成的 3D 图：
    // 渲染层只负责把正方形位图摆在圆底板正中，所以这里守的是「确实走了 inset 那条路」——
    // 一旦被改成和四家平台一样的铺满裁圆，透明底卡片会被拉到圆边上，四角出圆。
    const b = brandOf('白条')
    expect(b.image).toMatch(/brand\/credit-v\d+\.png$/)
    expect(b.imageInset).toBe(true)
    expect(b.plate).toBe('light')
    // 缩放比：太小看不清卡面，太大贴到圆边
    expect(b.scale).toBeGreaterThanOrEqual(0.7)
    expect(b.scale).toBeLessThanOrEqual(0.85)
    expect(brandOf('先用后付')).toBe(b)
  })
  ...
  it('白条平台四家都用官方位图，「白条」两个字用通用卡片', () => {
    ...
    // 四家平台铺满裁圆；「白条」分组是透明底卡片放在底板里，两条渲染路径不能混
    for (const n of ['京东白条', '拼多多', '美团月付', '花呗']) expect(brandOf(n).imageInset, n).toBeUndefined()
    // 单色自绘图标不带位图，否则 path 白画了
    for (const [k, b] of Object.entries(BRANDS)) expect(b.image, k).toBeUndefined()
    // 四家颜色（图表里的线和圆点用）各不相同
