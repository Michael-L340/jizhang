...
- 图表上**不属于分类**的颜色（趋势线 / 余额线 / 日均线 / 坐标轴）走 `palette.CHART`，页面里不许再出现写死的十六进制——`palette.test.ts` 对 `Home.tsx` 和 `Stats.tsx` 都有守卫，并且从 `index.css` 的 `@theme` 现读来对账 `CHART`。09-05 换暖色主题时统计页漏了一批冷色（`#e5484d` / `#1f9d55` / `#2f6fed` / 冷灰轴），在奶油底上格外脏，09-08 才收拾干净。
- 堆叠柱的左右两根轴用 `chart.pairedAxisMax` 算，目标是**让日均线正好压在柱顶的轮廓上**：日均 × 桶天数 = 桶总额，所以 `线轴上限 = 柱轴上限 ÷ 标准桶天数`（完整桶天数的中位数），标准长度的月份就精确落在柱顶，短月抬起来、长月压下去，**线和柱顶之间那道缝就是「这个月比标准月长多少」**。实测线高÷柱高：两轴各自 auto = 0.32（埋在柱子肚子里）、只对齐格数 = 0.82（还在中上部横切）、现在 = 0.94–1.10。柱轴先留一成头（`headroom` 0.88），`niceStep` 除了 1/2/5 还给了 1.5/3/4/6/8，不然 2360 会一口气跳到 5000、柱子只剩半屏高。
- **lineMax 只取完整的桶，本月不画进线里**。本月才过几天，÷ 已过天数的日均是完整月的三五倍，参与定量程就会把前面十一个月全压扁。本月的日均数字放在卡片上方那行小字和提示框里，一个都不少。
- 日均线底下垫一条 5px 的白线（`HALO` 那个 series），比 `shadowBlur` 干净也更稳；它不进图例、不进提示框，提示框的 formatter 按 series 名字滤掉。
