...
- 统计页「趋势」有三档：合计 / 分类 / **堆叠**（一级分类堆叠柱）。柱子上不标任何数字——12 根 × 5 段共 60 个数手机上放不下，金额和占比在提示框里。曾经在上面叠过一条「日均消费」的折线走右轴（2026-09-08 做了又撤）：无论怎么调量程它都在柱子上横切，用户看完决定不要；要是哪天想找回来，`git log` 里 b7b540e 那一版有完整实现和双轴对齐算法。
...
- 图表上**不属于分类**的颜色（趋势线 / 余额线 / 坐标轴）走 `palette.CHART`，页面里不许再出现写死的十六进制——`palette.test.ts` 对 `Home.tsx` 和 `Stats.tsx` 都有守卫，并且从 `index.css` 的 `@theme` 现读来对账 `CHART`。09-05 换暖色主题时统计页漏了一批冷色（`#e5484d` / `#1f9d55` / `#2f6fed` / 冷灰轴），在奶油底上格外脏，09-08 才收拾干净。
